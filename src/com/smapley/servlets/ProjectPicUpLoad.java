 ...
 * 
 */
@WebServlet("/ProjectPicUpLoad")
public class ProjectPicUpLoad extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		System.out.println("---ProjectPicUpLoad---");

		// 获得磁盘文件条目工厂
		DiskFileItemFactory factory = new DiskFileItemFactory();
		// 获取文件需要上传到的路径
		@SuppressWarnings("deprecation")
		String path = request.getRealPath("/upload/pic/project");
		...
		try {
			...
			User user = XDAO.userDAO.findById(Integer.parseInt(map
					.get("userId")));
			Project project=XDAO.projectDAO.findById(Integer.parseInt(map.get("projectId")));
			for (FileItem item : list) {
				if (!item.isFormField()) {
					// 获取表单的属性名字
					...
					project.setPicUrl("project/"+filename);
					project.setRefresh(new Timestamp(System.currentTimeMillis()));
					XDAO.projectDAO.attachDirty(project);
				}
			}
